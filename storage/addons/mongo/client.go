...
func newClient(client *mongo.Client) *mongoClient {
	return &mongoClient{
		Client: client,
	}
}

func (c *mongoClient) Truncate(database string, collection string) error {
	cl := c.Client.Database(database).Collection(collection)
	return cl.Drop(context.Background())
}

func (c *mongoClient) InsertDocuments(database string, collection string, documents []map[string]interface{}) ([]map[string]interface{}, error) {
	cl := c.Client.Database(database).Collection(collection)
