...
https://pkg.go.dev/github.com/lansfy/gonkex[image:https://pkg.go.dev/badge/github.com/lansfy/gonkex.svg[Go Reference]] https://github.com/lansfy/gonkex/blob/master/.testcoverage.yml[image:https://raw.githubusercontent.com/lansfy/gonkex/refs/heads/badges/.badges/master/coverage.svg[coverage]]

[NOTE]
====
You can find a utility that allows you to run gonkex scripts https://github.com/lansfy/gonkex-cli[here] (in the "Releases" section).
====
...
* run as a https://github.com/lansfy/gonkex-cli/[standalone tool] or as a link:#using-gonkex-as-a-library[library] alongside your unit tests
* stores the results as an https://allurereport.org/[Allure] report
...
To start using the custom storage, you need to import the custom module, that contains implementation of https://pkg.go.dev/github.com/lansfy/gonkex/storage#StorageInterface[storage.StorageInterface] interface.
For example, the following NoSQL databases are currently supported as custom modules:

* Aerospike (https://github.com/lansfy/gonkex/tree/master/storage/addons/aerospike[storage/addons/aerospike])
* MongoDB (https://github.com/lansfy/gonkex/tree/master/storage/addons/mongo[storage/addons/mongo])
* Redis (https://github.com/lansfy/gonkex/tree/master/storage/addons/redis[storage/addons/redis])
...
where `+regular_expression+` is a valid https://pkg.go.dev/regexp/syntax[Go regular expression] pattern.
...
* `+format_string+` is a valid https://pkg.go.dev/time#pkg-constants[Go time format] or https://pkg.go.dev/github.com/ncruces/go-strftime#pkg-overview[strftime time format] pattern - optional parameters can be added to customize the time matching behavior
...
[NOTE]
====
`+duration+` should be defined using https://pkg.go.dev/time#ParseDuration[Go time duration string]. For convenience, days (`+d+`) and weeks (`+w+`) are also supported.
====
...
[NOTE]
====
`+offset+` should be defined using https://pkg.go.dev/time#ParseDuration[Go time duration string]. For convenience, days (`+d+`) and weeks (`+w+`) are also supported.
====
...
This delays should be defined using https://pkg.go.dev/time#ParseDuration[Go time duration string].
...
All paths must be specified in https://github.com/tidwall/gjson/blob/master/SYNTAX.md[gjson format]. You can use the https://gjson.dev[GJSON Playground] to experiment with the syntax online.
...
----
  ...
  mocks:
    service1:
      requestConstraints:
        # this check will demand that the request contains key1 Рё key2
        # and the values are key1=value1, key1=value11 Рё key2=value2.
...
This strategy gives ability to use incoming request data into mock response. Implemented with package https://pkg.go.dev/text/template[text/template].
...
* `+timeout+` - time is responsible for stopping the script on timeout. Should be specified in https://pkg.go.dev/time#ParseDuration[Go time duration string] or in seconds. The default value is `+3s+`.
...
Use https://raw.githubusercontent.com/lansfy/gonkex/master/schema/gonkex.json[file with schema] to add syntax highlight to your favourite IDE and write Gonkex tests more easily.
It adds in-line documentation and auto-completion to any IDE that supports it. The https://github.com/lansfy/gonkex/tree/master/schema[following article] describes how to add schema to your IDE.
